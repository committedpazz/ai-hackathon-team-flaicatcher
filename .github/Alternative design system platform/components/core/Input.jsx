  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8, ...style }}>
      ...
      <input
        ...
        onFocus={(e) => { if (!invalid) e.target.style.borderColor = "var(--ce-navy-700)"; }}
        onBlur={(e) => { if (!invalid) e.target.style.borderColor = "var(--ce-divider-on-light)"; }}
        {...rest}
      />
      {hint && (
        <span style={{ fontSize: 13, color: invalid ? "#d64545" : "var(--ce-gray-400)" }}>{hint}</span>
      )}
    </div>
  );
}
